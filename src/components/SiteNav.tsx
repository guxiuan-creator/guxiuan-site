...
export function SiteNav() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-zinc-950/15 bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/70">
      <div className="mx-auto flex w-full max-w-7xl items-center justify-between px-4 py-5 sm:px-8">
        <Link href="/" className="group inline-flex items-baseline gap-3">
          ...
          <span className="hidden text-xs text-zinc-500 sm:inline">ToC PM</span>
        </Link>
        ...
        <button
          ...
          className="inline-flex items-center rounded-full border border-zinc-950/10 bg-white px-4 py-2 text-xs font-semibold tracking-[0.22em] text-zinc-700 hover:bg-zinc-50 sm:hidden"
      ...
      {open ? (
        <div className="fixed inset-0 z-50 sm:hidden">
          ...
          <div className="absolute right-0 top-0 h-dvh w-[86%] max-w-sm border-l border-zinc-950/10 bg-white shadow-[0_30px_120px_-70px_rgba(0,0,0,0.65)]">
            <div className="flex items-center justify-between border-b border-zinc-950/10 px-5 py-4">
              <div className="flex items-baseline gap-3">
                <span className="text-xs font-semibold tracking-[0.28em] text-zinc-500">
                  MENU
                </span>
                <span className="text-xs text-zinc-500">GUXIUAN</span>
              </div>
              <button
                type="button"
                aria-label="Close"
                onClick={() => setOpen(false)}
                className="rounded-full border border-zinc-950/10 bg-white px-3 py-1 text-xs font-semibold tracking-[0.18em] text-zinc-700"
              >
                CLOSE
              </button>
            </div>
  ...
  return (
    <Link
      ...
      className={[
        ...
        active
          ? "border-zinc-950/15 bg-zinc-950 text-white"
    ...
    >
      <div className="space-y-1">
        ...
        <div className={["text-base font-semibold", active ? "text-white" : ""].join(" ")}>
          {labelZh}
        </div>
      </div>
      <span className={["text-sm", active ? "text-white/70" : "text-zinc-400"].join(" ")}>
        →
      </span>
    </Link>
  );
